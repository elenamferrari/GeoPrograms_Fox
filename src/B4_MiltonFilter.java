public class B4_MiltonFilter extends World {
   ...
   public void go() {
      plane.loadBackGround("axolotl.jpg");          //*** new method to load a diffrent JPG
      System.out.println("This will be printed to the  window. ");
      plane.teleport(0, 0);
      plane.showBackGround();
      System.out.println(plane.howMuchRed());

      for (int row = 215; row < 350; row++)    //*** this is using new getBackground
      {
         for (int col = 160; col < 350; col++)    //*** this is using new getBackground
         {
            ...
            /*if(red>200 && green >130 && blue>130) //&&green<240 && blue< 100)
            {
               plane.setPixelColor(0,250,1);
            }
            
            if((red>110 && green>110 && blue>110) && (red<130 && green<130 && blue<130))
            {
               plane.setPixelColor(230,70,240);
            }

            if(red<1 && green<1 && blue<1)
            {
               plane.setPixelColor(10,200,200);
             */
            if ((red >220 && green >220 && blue >220) && (red<=255 && green<=256 && blue<235)) {
               plane.setPixelColor(0, 250, 1);
            }
            {
            }
